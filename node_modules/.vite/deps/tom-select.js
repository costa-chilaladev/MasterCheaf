...
var setToPattern = (chars) => {
	return arrayToPattern(Array.from(chars));
};
/**
...
//#endregion
//#region node_modules/@orchidjs/unicode-variants/dist/esm/strings.js
/**
...
//#endregion
//#region node_modules/@orchidjs/unicode-variants/dist/esm/index.js
...
var convert_pat = new RegExp(Object.keys(latin_convert).join("|") + "|[̀-ͯ·ʾʼ]", "gu");
/**
...
var inSequences = (needle_seq, sequences) => {
	for (const seq of sequences) {
		...
		if (seq.parts.filter(filter).length > 0) continue;
		return true;
	}
	return false;
};
...
var getPattern = (str) => {
	...
	for (let i = 0; i < str.length; i++) {
		let match = str.substring(i).match(multi_char_reg);
...
//#endregion
//#region node_modules/@orchidjs/sifter/dist/esm/utils.js
/**
...
*
*/
var iterate$5 = (object, callback) => {
	if (Array.isArray(object)) object.forEach(callback);
	else for (var key in object) if (object.hasOwnProperty(key)) callback(object[key], key);
};
var cmp = (a, b) => {
...
//#endregion
//#region node_modules/@orchidjs/sifter/dist/esm/sifter.js
/**
* sifter.js
...
var Sifter = class {
		...
		*/
		const scoreObject = (function() {
			...
			return function(token, data) {
				...
				} else iterate$5(weights, (weight, field) => {
	...
	*/
	...
	_getSortFunction(search) {
		...
		*/
		...
		if (!sort_flds.length) return null;
		return function(a, b) {
			var result, field;
			for (let sort_fld of sort_flds) {
				field = sort_fld.field;
				result = (sort_fld.direction === "desc" ? -1 : 1) * cmp(get_field(field, a), get_field(field, b));
	...
	*/
	search(query, options) {
		var self = this, score, search = this.prepareSearch(query, options);
		options = search.options;
		query = search.query;
		const fn_score = options.score || self._getScoreFunction(search);
		if (query.length) iterate$5(self.items, (item, id) => {
			score = fn_score(item);
		...
		else iterate$5(self.items, (_, id) => {
			search.items.push({
				"score": 1,
				"id": id
			});
...
//#endregion
//#region node_modules/tom-select/dist/esm/utils.js
/**
...
*
*/
var hash_key$1 = (value) => {
	if (typeof value === "undefined" || value === null) return null;
	return get_hash$1(value);
};
var get_hash$1 = (value) => {
...
*
*/
var escape_html$1 = (str) => {
	return (str + "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
};
/**
* use setTimeout if timeout > 0
*/
var timeout = (fn, timeout) => {
	if (timeout > 0) return window.setTimeout(fn, timeout);
	fn.call(null);
	return null;
};
/**
...
*
*/
var preventDefault$5 = (evt, stop = false) => {
	if (evt) {
		evt.preventDefault();
...
*
*/
var addEvent$5 = (target, type, callback, options) => {
...
var isKeyDown = (key_name, evt) => {
	...
	if ((evt.altKey ? 1 : 0) + (evt.ctrlKey ? 1 : 0) + (evt.shiftKey ? 1 : 0) + (evt.metaKey ? 1 : 0) === 1) return true;
	return false;
};
/**
...
*
*/
var iterate$4 = (object, callback) => {
	if (Array.isArray(object)) object.forEach(callback);
	else for (var key in object) if (object.hasOwnProperty(key)) callback(object[key], key);
};
//#endregion
//#region node_modules/tom-select/dist/esm/vanilla.js
/**
...
*
*/
var addClasses$2 = (elmts, ...classes) => {
...
*
*/
var removeClasses$1 = (elmts, ...classes) => {
	var norm_classes = classesArray$3(classes);
	elmts = castAsArray$3(elmts);
	elmts.map((el) => {
		norm_classes.map((cls) => {
...
*
*/
var classesArray$3 = (args) => {
	var classes = [];
	iterate$4(args, (_classes) => {
		if (typeof _classes === "string") _classes = _classes.trim().split(/[\t\n\f\r\s]/);
...
*
*/
var parentMatch$1 = (target, selector, wrapper) => {
...
//#endregion
//#region node_modules/tom-select/dist/esm/contrib/highlight.js
/**
...
var KEY_SHORTCUT = (typeof navigator === "undefined" ? false : /Mac/.test(navigator.userAgent)) ? "metaKey" : "ctrlKey";
//#endregion
//#region node_modules/tom-select/dist/esm/defaults.js
	...
	*/
	var init_select = () => {
		...
		var addOption = (option, group) => {
			...
			if (optionsMap.hasOwnProperty(value)) {
				if (group) {
					...
					else if (!Array.isArray(arr)) optionsMap[value][field_optgroup] = [arr, group];
					else arr.push(group);
				}
			} else {
		...
		var addGroup = (optgroup) => {
			var id, optgroup_data = readData(optgroup);
...
var TomSelect = class extends MicroPlugin(MicroEvent) {
	constructor(input_arg, user_settings) {
		...
		dir = (window.getComputedStyle && window.getComputedStyle(input, null)).getPropertyValue("direction");
		...
		if (settings.copyClassesToDropdown) addClasses$2(dropdown, classes);
		...
		if (isHtmlString$6(settings.controlInput)) {
			...
			iterate$4([
				...
				if (input.getAttribute(attr)) setAttr$1(control_input, { [attr]: input.getAttribute(attr) });
			});
	...
	*/
	setup() {
		...
		setAttr$1(dropdown_content, { id: listboxId });
		...
		if ((settings.maxItems === null || settings.maxItems > 1) && self.is_select_tag) setAttr$1(input, { multiple: "multiple" });
		if (settings.placeholder) setAttr$1(control_input, { placeholder: settings.placeholder });
		...
		const doc_mousedown = (evt) => {
			...
			if (target == control_input && self.isOpen) evt.stopPropagation();
			else preventDefault$5(evt, true);
		};
		const win_scroll = () => {
		...
		else if (input.readOnly) self.setReadOnly(true);
		else self.enable();
	...
	*/
	onPaste(e) {
		...
		setTimeout(() => {
			...
			iterate$4(pastedText.trim().split(self.settings.splitOn), (piece) => {
				if (hash_key$1(piece)) if (this.options[piece]) self.addItem(piece);
	...
	*/
	onKeyDown(e) {
		...
		if (self.isLocked) {
			if (e.keyCode !== 9) preventDefault$5(e);
			return;
		}
		switch (e.keyCode) {
			case 65:
			...
			case 13:
				if (self.canSelect(self.activeOption)) {
					...
					preventDefault$5(e);
				} else if (self.settings.create && self.createItem()) preventDefault$5(e);
				else if (document.activeElement == self.control_input && self.isOpen) preventDefault$5(e);
				return;
			case 37:
		...
		if (self.isInputHidden && !isKeyDown(KEY_SHORTCUT, e)) preventDefault$5(e);
	}
	/**
	...
	*/
	onOptionSelect(evt, option) {
		...
		if (option.classList.contains("create")) self.createItem(null, () => {
			...
			else if (self.settings.clearAfterSelect) self.setTextboxValue();
		});
		else {
	...
	*/
	loadCallback(options, optgroups) {
		...
		if (!self.loading) removeClasses$1(self.wrapper, self.settings.loadingClass);
	...
	*/
	setActiveItem(item, e) {
		...
		if (eventName === "click" && isKeyDown("shiftKey", e) && self.activeItems.length) {
			...
			preventDefault$5(e);
		} else if (eventName === "click" && isKeyDown(KEY_SHORTCUT, e) || eventName === "keydown" && isKeyDown("shiftKey", e)) if (item.classList.contains("active")) self.removeActiveItem(item);
	...
	*/
	inputState() {
		...
		if (self.activeItems.length > 0 || !self.isFocused && self.settings.hidePlaceholder && self.items.length > 0) {
			...
			if (self.settings.hidePlaceholder && self.items.length > 0) setAttr$1(self.control_input, { placeholder: "" });
	...
	*/
	getSearchOptions() {
		...
		if (typeof settings.sortField === "string") sort = [{ field: settings.sortField }];
		return {
	...
	refreshOptions(triggerDropdown = true) {
		...
		for (i = 0; i < n; i++) {
			...
			for (j = 0, k = optgroups && optgroups.length; j < k; j++) {
				...
				if (self_optgroup === void 0) optgroup = "";
				else order = self_optgroup.$order;
		...
		iterate$4(groups_order, (group_order) => {
			...
			if (group_heading !== void 0) {
				...
				append(group_options, self.render("optgroup_header", group_heading));
		...
		if (self.settings.highlight) {
			...
			if (results.query.length && results.tokens.length) iterate$4(results.tokens, (tok) => {
		...
		if (self.loading) add_template("loading");
		else if (!self.settings.shouldLoad.call(self, query)) add_template("not_loading");
	...
	addItem(value, silent) {
		...
		debounce_events(this, events, () => {
			...
			if (self.isSetup) {
				...
				if (self.settings.clearAfterSelect) self.setTextboxValue();
				...
				if (!self.isPending) self.updateOriginalInput({ silent });
			}
	...
	*/
	createItem(input = null, callback = () => {}) {
		...
		if (!self.canCreate(input)) {
			if (hash_key$1(input)) {
				if (this.options[input]) self.addItem(input);
			}
			callback();
			return false;
		}
	...
	updateOriginalInput(opts = {}) {
		...
		if (self.is_select_tag) {
			...
			function AddSelected(option_el, value, label) {
				if (!option_el) option_el = getDom$6("<option value=\"" + escape_html$1(value) + "\">" + escape_html$1(label) + "</option>");
			...
			if (self.items.length == 0 && self.settings.mode == "single") AddSelected(empty_option, "", "");
			else self.items.forEach((value) => {
				option = self.options[value];
				label = option[self.settings.labelField] || "";
				if (selected.includes(option.$option)) AddSelected(self.input.querySelector(`option[value="${addSlashes(value)}"]:not(:checked)`), value, label);
	...
	*/
	clear(silent) {
		...
		iterate$4(self.controlChildren(), (item) => {
	...
	*/
	shouldDelete(items, evt) {
		...
		if (!values.length || typeof this.settings.onDelete === "function" && this.settings.onDelete.call(this, values, evt) === false) return false;
		return true;
	}
	/**
	...
	*/
	advanceSelection(direction, e) {
		...
		if (isKeyDown(KEY_SHORTCUT, e) || isKeyDown("shiftKey", e)) {
			...
			if (last_active) if (!last_active.classList.contains("active")) adjacent = last_active;
	...
	*/
	render(templateName, data) {
		...
		if (!html) return null;
		html = getDom$6(html);
		if (templateName === "option" || templateName === "option_create") if (data[self.settings.disabledField]) setAttr$1(html, { "aria-disabled": "true" });
		else setAttr$1(html, { "data-selectable": "" });
		else if (templateName === "optgroup") {
			id = data.group[self.settings.optgroupValueField];
			setAttr$1(html, { "data-group": id });
			if (data.group[self.settings.disabledField]) setAttr$1(html, { "data-disabled": "" });
		}
...
//#endregion
//#region node_modules/tom-select/dist/esm/plugins/change_listener/plugin.js
/**
* Tom Select v2.6.0
...
*
*/
var addEvent$4 = (target, type, callback, options) => {
	target.addEventListener(type, callback, options);
};
/**
...
*
*/
var preventDefault$4 = (evt, stop = false) => {
	if (evt) {
		evt.preventDefault();
		if (stop) evt.stopPropagation();
	}
...
*
*/
function plugin$12(userOptions) {
	var self = this;
...
*
*/
function plugin$11(userOptions) {
	const self = this;
...
*
*/
var preventDefault$3 = (evt, stop = false) => {
	if (evt) {
		evt.preventDefault();
		if (stop) evt.stopPropagation();
	}
...
*
*/
var addEvent$3 = (target, type, callback, options) => {
	target.addEventListener(type, callback, options);
};
/**
* Iterates over arrays and hashes.
	...
	self.hook("after", "setupTemplates", () => {
		...
		self.settings.render.item = (data, escape) => {
			...
			const moveitem = (targetitem, dragitem) => {
				...
				if (isBefore(dragitem, item)) insertAfter(targetitem, dragitem);
				else insertBefore(targetitem, dragitem);
			};
			const dragend = () => {
				var _drag_item;
...
*
*/
var preventDefault$2 = (evt, stop = false) => {
	if (evt) {
		evt.preventDefault();
		if (stop) evt.stopPropagation();
	}
...
*
*/
var preventDefault$1 = (evt, stop = false) => {
	if (evt) {
		evt.preventDefault();
		if (stop) evt.stopPropagation();
	}
...
*
*/
var addEvent$2 = (target, type, callback, options) => {
	target.addEventListener(type, callback, options);
};
/**
* Iterates over arrays and hashes.
...
*
*/
var addClasses$1 = (elmts, ...classes) => {
...
*
*/
var addEvent$1 = (target, type, callback, options) => {
	target.addEventListener(type, callback, options);
};
/**
...
function plugin$1(userOptions) {
	const self = this;
	const options = Object.assign({ text: (option) => {
...
function plugin() {
	...
	if (!self.settings.shouldLoadMore) self.settings.shouldLoadMore = () => {
		if (dropdown_content.clientHeight / (dropdown_content.scrollHeight - dropdown_content.scrollTop) > .9) return true;
		if (self.activeOption) {
			var selectable = self.selectable();
			if (Array.from(selectable).indexOf(self.activeOption) >= selectable.length - 2) return true;
		}
		return false;
	};
	...
	self.hook("before", "refreshOptions", () => {
		if (self.activeOption && "option" !== self.activeOption.getAttribute("role")) self.setActiveOption(self.activeOption.previousElementSibling);
	});
	self.hook("after", "refreshOptions", () => {
		...
		} else if (query in pagination && !dropdown_content.querySelector(".no-results")) option = self.render("no_more_results", { query });
		if (option) {
...
//#endregion
//#region node_modules/tom-select/dist/esm/tom-select.complete.js
TomSelect.define("change_listener", plugin$13);
TomSelect.define("checkbox_options", plugin$12);
TomSelect.define("clear_button", plugin$11);
TomSelect.define("drag_drop", plugin$10);
...
//#endregion
export { tom_select_complete_default as default };
